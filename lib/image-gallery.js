...
var getGalleryWidth = require('./util/gallery-width'),
    selectAll = require('./util/select-all'),
...
function ImageGallery(selector, options) {
  ...
  options = xtend({}, options, DEFAULT_OPTIONS);
...
ImageGallery.prototype.layout = function(gallery) {
  ...
  var containerWidth = gallery.width = getGalleryWidth(gallery);

  var result = layout(gallery.layoutData, {
    align: 'center',
    containerWidth: containerWidth,
    idealElementHeight: gallery.imageHeight,
    spacing: gallery.imageSpacing || 0
  });
